...
class PPO:
    ...
    def select_action(self, state, available_actions=None):
        ...
        if self.has_continuous_action_space:
            with torch.no_grad():
                ...
                action, action_logprob, state_val = self.policy_old.act(state, available_actions)
    ...
    def update(self, tdzero=True):
        ...
        old_states_next =  torch.squeeze(torch.stack(self.buffer.states_next, dim=0)).detach().to(device)
        ...
        for _ in range(self.K_epochs):
            ...
            #loss = -torch.min(surr1, surr2) + self.MseLoss(state_values, returns)
