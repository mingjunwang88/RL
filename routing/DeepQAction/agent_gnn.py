...
import torch_geometric.transforms as pgy_T
...
class Critic(nn.Module):
    def __init__(self, state_dim, h1, h2, alpha=1e-4, dim_embed=36,):
        #######################################################################
        # 1: Each node is represented by a embedding obtained from the GNN node
        # 2: Only topology specific, not generlaize to other topologies.
        ...
        self.gcn1 = pyg_nn.GATConv(state_dim,dim_embed)
        self.gcn2 = pyg_nn.GATConv(dim_embed, dim_embed)
        self.glinear = nn.Linear(dim_embed,dim_embed)
        
        self.model = pyg_nn.Sequential('x, edge_index', [
            (pyg_nn.GATConv(state_dim, dim_embed), 'x, edge_index -> x'),
            nn.Tanh(),
            (pyg_nn.GCNConv(dim_embed, dim_embed), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
            nn.Linear(dim_embed, dim_embed),
        ])
          
        self.linear1 = nn.Linear(3*dim_embed, h1)
    ...
    def gnn_forward(self, x, edge_index):
        ####################################################
        # data: a pytorch geometric Data obj
        # src, dest: torch tensor
        ####################################################
        x = x.to(self.device)
        ...
        #return the embeeding
        return x, edge_index
    
    def forward(self, state, action):
        ####################################################
        # data: a torch geometric Data obj
        # src,dest: torch tensor
        ####################################################
...
class Agent():
    def __init__(self, env, num_state, dim_state, dim_action,h1, h2, max_memsize, alpha=1e-4, batch_size=258, max_iters = 30, \
                 epsilon=0.05, steps_target=1000, gamma=0.99):
        ...
        self.loss_fun = nn.MSELoss()
                           #state_dim, h1, h2, alpha=1e-4, dim_embed=36
        self.model = Critic(num_state,h1, h2, alpha=1e-4,dim_embed=dim_state).to(self.device)
        self.model_target = Critic(num_state, h1, h2, alpha=1e-4,dim_embed=dim_state).to(self.device)
    ...
    def act(self, current_pos):
        ...
        if np.random.rand() < self.epsilon:
            ...
            src = self.node_embed[T.tensor(current_pos).repeat(n)].to(self.device)
            dest = self.node_embed[T.tensor(self.env.dest).repeat(n)].to(self.device)
            
            actions_tensor = self.node_embed[T.tensor(actions, dtype=T.long)].to(self.device)
            inpt = T.cat((src,dest), dim=1)
            
            Qs = self.model.forward(inpt,actions_tensor)
            if len(Qs)>0:
                best = T.argmax(Qs)
        ...
        for i in range(self.max_iters):
            ...
            with torch.autograd.set_detect_anomaly(True):
                while not done:
                    ...
                    #Obtain the update version of node embeding
                    self.node_embed, edge_index = self.model.gnn_forward(self.env.graph.x, self.env.graph.edge_index)
                    ...
                    state_embd = self.node_embed[T.tensor(state)].view(1,-1) #including src and dest
                    action_embed = self.node_embed[action]
                    state_new_embed = self.node_embed[T.tensor(state_new)].view(1,-1)
                    action_new_embd = self.node_embed[action_new]
                    ...
                    if self.buffer.counter > self.batch_size:
                        ...
                        # Q value
                        q_values = T.squeeze(self.model.forward(state_batch,action_batch))
