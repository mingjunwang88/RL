...
class Model(nn.Module):
    def __init__(self, num_state, h1, h2, alpha=1e-4):
        ...
        self.linear1 = nn.Linear(2*num_state, h1)
...
class Agent():
    def __init__(self, env, num_state, h1, h2, max_memsize, alpha=1e-4, batch_size=258, max_iters = 30, epsilon=0.05, steps_target=1000, gamma=0.99, min_epsilon=0.005): 
    ...
    def train(self,):
        ...
        for i in tqdm(range(self.max_iters)): 
            ...
            for state in range(self.env.num_devices):
                ...
                while not done:
                    its+=1
                    if its > self.env.num_devices * 10:
                        break
                    ...
                    state_embd = self.one_hot(self.num_state, state)
                    action_embed = self.one_hot(self.num_state, action.item())
                    state_new_embed = self.one_hot(self.num_state, state)
    ...
    def find_path(self,src):
        ...
        if src == self.env.dest:
            print('Src and dest are same')
            return 
        
        while s != self.env.dest:
            iters+=1
            
            if iters == 2*self.env.num_devices:
                print('No path found')
                break
