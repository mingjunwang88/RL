...
class State(object):
    def __init__(self, warehouse_num, T, demand_history, t = 0):
        ###############################################################
        #State: 1: Current stock levels for each warehouse
        #       2: historical demend record for several time period for each warehouse (assume that the agent observes only past demand values)
        #       3: factory_stock
        ###############################################################
...
class SupplyChainEnvironment(object):
    def __init__(self):
        ...
        self.d_max = 5.0            # maximum demand, units
        self.d_var = 2.0           # maximum random demand variation, units 
    
        self.unit_price = 100.0     # unit price in dollars
        self.unit_cost = 40.0       # unit cost in dollars

        self.storage_capacities = np.fromfunction(lambda j: 10*(j+1), (self.warehouse_num + 1,), dtype=np.int)
        
        # storage costs at the factory and each warehouse, dollars per unit
        self.storage_costs = np.fromfunction(lambda j: 2*(j+1), (self.warehouse_num + 1,), dtype=np.int)  
        
        # transportation costs for each warehouse, dollars per unit
        self.transporation_costs = np.fromfunction(lambda j: 5*(j+1), (self.warehouse_num,), dtype=np.int) 
    ...
    def demand(self, j, t):
        ...
        return self.d_max/2.0 + self.d_max/2*np.sin(2*np.pi*(t + 2*j)/self.T*2) + np.random.randint(0, self.d_var)

    def initial_state(self):
        return State(self.warehouse_num, self.T, list(self.demand_history))

    def step(self,state, action):
        demands = np.fromfunction(lambda j: self.demand(j+1, self.t), (self.warehouse_num,), dtype=np.int)
...
class SimpleSupplyChain(gym.Env):
    def __init__(self, config=None):
        self.reset()
        self.action_space = Box(low=0.0, high=20.0, shape=(self.supply_chain.warehouse_num + 1, ))
        self.observation_space = Box(-10000, 10000, shape=(len(self.supply_chain.initial_state().to_array()), ))
