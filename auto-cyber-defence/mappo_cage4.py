...
from agents.agent_mappo_cage4 import Agent
import os
...
class Mappo:
    def __init__(self, env, dim_state: int, lr_actor, lr_critic, gamma, K_epochs, eps_clip, has_continuous_action_space, action_std_init, steps_update,
        ...
        for agent_index, agent in enumerate(env.agents):
            ...
            self.agents[agent] = Agent(dim_state, sum([i.n for i in env.observation_space(agent)]), self.num_actions[agent], lr_actor, lr_critic, gamma, K_epochs, eps_clip, 
    ...
    def select_actions(self, obses: Dict, all_state: np.array) -> Dict:
        ...
        for agent in self.env.agents:
            ...
            actions[agent] = self.agents[agent].select_action(obs_numeric, all_state)
        return actions

    def plot_learning_curve(self,x, scores, figure_file='output/mappo_cage4'):
    ...
    def learn(self, maxm_Iters, evaluate=False):
            ...
            for i in range(maxm_Iters):
                ...
                while not any(done_list):
                    ...
                    actions = self.select_actions(obs_, all_state)
                    ...
                    #print('done: ', done_list, any(done_list))                       
                    obs = obs_next
            ...
            writer.close()
            with open('output/mappo/'+ f'mappo_indep_{self.tag}', 'wb') as f:
...
#            env, dim_state, lr_actor, lr_critic, gamma, K_epochs, eps_clip, has_continuous_action_space, action_std_init, steps_update,
                 #tag, chkpt_dir=os.path.dirname(__file__)):
for tag in range(25):
    mappo = Mappo(env, DIM_STATE, 1e-4,     1e-4,      0.95,  40,       0.2,      False,                        0.6,             100, tag)
