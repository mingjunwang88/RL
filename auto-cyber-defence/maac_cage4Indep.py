...
from agents.agent_maac_cage4_indep import Agent
import os
import pickle
...
class Maac:
    def __init__(self, env, fc1, gamma, lr, tag, chkpt_dir=os.path.dirname(__file__)):
        """
        ...
        for agent_index, agent in enumerate(env.agents):
            ...
            #                          dim_obs,                                          num_action ,             fc1, gamma, lr, chkpt_dir=None, agent_idx=None):
            self.agents[agent] = Agent(sum([i.n for i in env.observation_space(agent)]), self.num_actions[agent], fc1, gamma, lr, chkpt_dir, agent_index)
    ...
    def plot_learning_curve(self,x, scores, figure_file='output/maac_cyborg_returnInd'):
    ...
    def learn(self, maxm_Iters, evaluate=False):
            ...
            with open('output/maac/'+ f'maac_indep_{self.tag}', 'wb') as f:
...
#           env, fc1, gamma,      lr,      tag,  chkpt_dir='output/models/'):
for tag in range(25):
    maac = Maac(env, 126, gamma=0.99, lr=0.02, tag=tag)    #3e-3
    maac.learn(maxm_Iters=1000, evaluate=False)
    #maac.load_check_point()

#python3 -m CybORG.Evaluation.evaluation --max-eps 2 . /tmp/output
# tensorboard --logdir=runsq
